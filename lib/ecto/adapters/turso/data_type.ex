if Code.ensure_loaded?(Ecto.Migration) do
  defmodule Ecto.Adapters.Turso.DataType do
    @moduledoc false

    # Simple column types. Note that we ignore options like :size, :precision,
    # etc. because columns do not have types, and Turso will not coerce any
    ...
    def column_type(:array, _opts) do
      case Application.get_env(:ex_turso, :array_type, :string) do
        :string -> "TEXT"
        :binary -> "BLOB"
      end
    end
    ...
    def column_type(:map, _opts) do
      case Application.get_env(:ex_turso, :map_type, :string) do
        :string -> "TEXT"
        :binary -> "BLOB"
      end
    end
