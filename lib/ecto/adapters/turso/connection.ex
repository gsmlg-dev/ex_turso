if Code.ensure_loaded?(Ecto.Adapters.SQL.Connection) do
  defmodule Ecto.Adapters.Turso.Connection do
    ...
    def child_spec(options) do
      ...
      DBConnection.child_spec(ExTurso.Connection, options)
    end
    ...
    def prepare_execute(conn, name, sql, params, options) do
      ...
      case DBConnection.execute(conn, query, params, options) do
        ...
        {:error, %ExTurso.Error{}} = error -> error
        {:error, err} -> raise err
      end
    end

    @impl true
    def execute(conn, %ExTurso.Query{} = query, params, options) do
      case DBConnection.execute(conn, query, params, options) do
        {:ok, returned_query, result} -> {:ok, returned_query, to_ecto_result(result)}
    ...
    defp build_query(sql, name \\ "") do
      ...
      %ExTurso.Query{name: name, statement: statement, command: command_for(statement)}
    end
    ...
    defp to_ecto_result(%ExTurso.Result{columns: columns, rows: rows, num_rows: num_rows}) do
    ...
    @impl true
    def to_constraints(
          %ExTurso.Error{message: "UNIQUE constraint failed: index " <> constraint},
          _opts
        ) do
    ...
    def to_constraints(
          %ExTurso.Error{message: "UNIQUE constraint failed: " <> constraint},
          _opts
        ) do
      [unique: constraint_name_hack(constraint)]
    end

    def to_constraints(%ExTurso.Error{message: "FOREIGN KEY constraint failed"}, _opts) do
      # unfortunately we have no other data from Turso
      [foreign_key: nil]
    end

    def to_constraints(
          %ExTurso.Error{message: "CHECK constraint failed: " <> name},
          _opts
        ) do
      [check: name]
    end
